...
- **アクション選択ダイアログ**: `Enter`で利用可能なアクションを一覧表示
- **Early worktree access**: フック実行中でもAI/エディタを開ける
...
`n` キーで作成ダイアログを開きます。**ステップベースのフロー**で進行します：

1. **ブランチ名**: 新しいworktreeのブランチ名を入力
2. **ベースブランチ**: 分岐元を選択
3. **オプション**: 「エディタを開く」チェックボックスを切り替え
4. **作成中**: フックの実行状況を確認
...
**Early worktree access**: フック実行中（mise install, bun installなど）でも、`e`でエディタを開いたり`a`でAIコマンドをコピーできます。完了を待つ必要なし！
